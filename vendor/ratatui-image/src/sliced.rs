        ...
        Self: Sized,
    {
        ...
        match &self.sliced_protocol {
            ...
            SlicedProtocol::Sliced(slices) => {
                // The iterator already removes the rows clipped from the top. Limit what
                // remains to the visible height; subtracting only the bottom rows from the
                // original length overdraws when the image is clipped at both ends.
                let visible_rows = usize::from(image_area.height);
                let mut image_area = image_area;
                image_area.height = 1;
                for slice in slices.iter().skip(skip_line_count).take(visible_rows) {
...
mod sixel_slice {
    ...
    impl<'a> SlicedSixelData<'a> {
        ...
        fn bands(&self, skip_line_count: usize, drop_line_count: usize) -> Vec<&str> {
            ...
            let end_bands = (((self.size.height.saturating_sub(drop_line_count as u16))
                * self.font_height)
                / 6) as usize;
            // `end_bands` is an absolute index from the image top, while `take` expects a
            // count after `skip_bands`. Using the absolute index as the count repeats the
            // skipped height below the viewport when both ends are clipped.
    ...
    mod tests {
        ...
        #[test]
        fn simultaneous_skip_and_drop_limits_bands_to_visible_rows() {
            let data = String::from("\x1bPq#0band0-band1-band2-band3-band4-\x1b\\");
            let sixel = Sixel {
                data,
                size: Size::new(1, 5),
                is_tmux: false,
            ...
            assert_eq!(
                sliced.borrow_dependent().bands(1, 1),
...
mod tests {
    use super::*;
    use crate::protocol::iterm2::Iterm2;
    use ratatui::{buffer::Buffer, widgets::Widget};

    #[test]
    fn simultaneous_skip_and_drop_limits_sliced_rows_to_the_area() {
        let rows = (0..7)
            .map(|row| {
            ...
            .collect();
        let protocol = SlicedProtocol::Sliced(rows);
        let mut buffer = Buffer::empty(Rect::new(0, 0, 1, 5));

        SlicedImage::new(&protocol, (0, -2).into()).render(Rect::new(0, 0, 1, 3), &mut buffer);

        assert_eq!(buffer[(0, 0)].symbol(), "2");
        assert_eq!(buffer[(0, 1)].symbol(), "3");
        assert_eq!(buffer[(0, 2)].symbol(), "4");
